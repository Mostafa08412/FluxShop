...
namespace FluxStore.Infrastructure.Authentication
{
    public sealed class IdentityService : IIdentityService
    {
        ...
        public async Task<Result> CreateUserAsync(string firstName, string lastName, string email, string password, CancellationToken cancellationToken, string role = Roles.User)
        {
            var user = await _userManager.FindByEmailAsync(email);

            if (user != null)
                return Result.Failure(Errors.IdentityErrors.EmailAlreadyExists);
        ...
        CancellationToken cancellationToken = default)
        {
            ...
            if (user == null)
            {
                return Result<(string, string)>.Failure(Errors.IdentityErrors.UserNotFoundByEmail(email));
            }
            ...
            if (isUserLocked)
            {
                return Result<(string, string)>.Failure(Errors.IdentityErrors.UserLockout);
            }



            if (_memoryCache.TryGetValue($"ResetPasswordOTP_{email}", out string _))
            {
                return Result<(string, string)>.Failure(Errors.IdentityErrors.WaitBeforeRequestingAnotherOtp());
            }
            else
            {
            ...
            _memoryCache.Set($"ResetPasswordOTP_{email}", otp, TimeSpan.FromSeconds(30));
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (user == null)
            {
                return Result<string>.Failure(Errors.IdentityErrors.UserNotFoundByEmail(email));
            }
            ...
            if (isUserLocked)
            {
                return Result<string>.Failure(Errors.IdentityErrors.UserLockout);
            }
            ...
            if (!isValid)
            {
                ...
                return Result<string>.Failure(Errors.IdentityErrors.InvalidOtp);
            }
            await _userManager.UpdateSecurityStampAsync(user);

            _memoryCache.Remove($"ResetPasswordOTP_{email}");
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (user == null)
            {
                return Result.Failure(Errors.IdentityErrors.UserNotFoundByEmail(email));
            }
            var resetResult = await _userManager.ResetPasswordAsync(user, resetToken, newPassword);

            if (!resetResult.Succeeded && resetResult.Errors.Any(X => X.Code == Errors.IdentityErrors.InvalidToken.Code))
            {
                return Result.Failure(Errors.IdentityErrors.InvalidPasswordResetToken);
            }
