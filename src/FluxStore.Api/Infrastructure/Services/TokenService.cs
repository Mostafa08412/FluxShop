...
namespace FluxStore.Api.Infrastructure.Services
{
    public sealed class TokenService
    {
        ...
        public (string, DateTime) GenerateAccessToken(ApplicationUser user, IList<string> roles, IList<Claim>? additionalClaims = null)
        {
            ...
            var expiresUtc = _dateTime.UtcNow.AddMinutes(_tokenSettings.AccessTokenExpiryInMinutes);
        ...
        public (string, DateTime) GenerateRefreshToken()
        {
            ...
            var expiresUtc = _dateTime.UtcNow.AddMinutes(_tokenSettings.RefreshTokenExpiryInMinutes);
       ...
       new SymmetricSecurityKey(Encoding.UTF8.GetBytes(_tokenSettings.SecretKey));
        ...
        private string BuildAccessToken(IEnumerable<Claim> claims)
        {
            var token = new JwtSecurityToken(
                ...
                claims: claims,
                expires: DateTime.UtcNow.AddMinutes(_tokenSettings.AccessTokenExpiryInMinutes),
